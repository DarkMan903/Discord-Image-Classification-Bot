...
async def hello(ctx):
    ...
    await ctx.send(f'3. join - совершить вход в голосовой канал' + '\n')
    await ctx.send(f'4. heh с указанием цифры или числа - я начинаю смеяться, но у меня есть секрет :)'+ '\n')
    await ctx.send(f'5. mem - кидаю рандомный мем' + '\n')
    await ctx.send(f'6. mem_rarity - кидаю мем по редкости' + '\n')
    await ctx.send(f'7. rec - выдаю рандомную рекомендацию' + '\n')
...
async def images(ctx):
    ...
    if attachments:
        for attachment in attachments:
            ...
            await attachment.save(f'image/{file_name}')
            await ctx.send(f'Сохранился в image/{file_name}')
            img = detect_maniak.image()
            if img.mode == 'RGB':
        ...
        await ctx.send('Забыли добавить картинку!')
